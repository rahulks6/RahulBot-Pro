...
import * as moderationActionsRepo from "./moderation-actions.repository";
...
async function suspendUser(userId: string): Promise<void> {
  ...
  await usersRepo.setAccountStatus(userId, "suspended");
...
export async function resolveReport(moderatorId: string, reportId: string, input: ResolveReportInput): Promise<ReportRow> {
  ...
  // The content/user action itself runs first, then the optimistic-locked
  // status write (moderationRepo.resolveReport's WHERE status = 'pending')
  // is the actual race-closer: if another moderator resolved this same
  // report in between our read above and this write, that update affects
  // zero rows and we 409 instead of silently double-actioning it. A
  // content removal or suspension that ran just before losing that race is
  // a harmless no-op re-application, not a corrupting side effect.
  if (input.action === "dismiss") {
    const won = await moderationRepo.resolveReport(reportId, moderatorId, "dismissed", input.note);
    if (!won) throw new HttpError(409, "This report was just resolved by someone else.");
    await moderationActionsRepo.record({ actorId: moderatorId, actionType: "dismiss_report", targetType: report.targetType, targetId: report.targetId, reportId, reason: input.note });
  } else if (input.action === "remove_content") {
    if (report.targetType === "story") {
      await storiesService.moderatorDeleteStory(report.targetId, moderatorId, input.note);
    } else if (report.targetType === "comment") {
      await engagementService.moderatorDeleteComment(report.targetId, moderatorId, input.note);
    } else {
      throw new HttpError(400, "remove_content only applies to story or comment reports.");
    }
    const won = await moderationRepo.resolveReport(reportId, moderatorId, "actioned", input.note);
    if (!won) throw new HttpError(409, "This report was just resolved by someone else.");
    await moderationActionsRepo.record({ actorId: moderatorId, actionType: "remove_content", targetType: report.targetType, targetId: report.targetId, reportId, reason: input.note });
  } else {
    // suspend_user
    if (report.targetType !== "user") {
    ...
    const won = await moderationRepo.resolveReport(reportId, moderatorId, "actioned", input.note);
    if (!won) throw new HttpError(409, "This report was just resolved by someone else.");
    await moderationActionsRepo.record({ actorId: moderatorId, actionType: "suspend_user", targetType: "user", targetId: report.targetId, reportId, reason: input.note });
  }
  await auditRepo.record({ actorId: moderatorId, action: `report.resolve.${input.action}`, targetType: report.targetType, targetId: report.targetId, metadata: { reportId } });
...
/**
 * The new, granular-permission-gated twin of the actions above — reachable
 * from the Admin Console (not the legacy mobile moderation queue), and
 * usable without an underlying report (a moderator acting on something
 * nobody reported yet). Writes the same moderation_status/moderation_actions/
 * audit_logs trail either way.
 */
export async function removeContent(actorId: string, targetType: TargetType, targetId: string, reason: string | null): Promise<void> {
  const actor = await permissionsService.requirePermission(actorId, "content.remove");
  if (targetType === "story") {
    await storiesService.moderatorDeleteStory(targetId, actor.id, reason);
  } else if (targetType === "comment") {
    await engagementService.moderatorDeleteComment(targetId, actor.id, reason);
  } else {
    throw new HttpError(400, "content.remove only applies to story or comment targets.");
  }
  await moderationActionsRepo.record({ actorId: actor.id, actionType: "remove_content", targetType, targetId, reason });
  await auditRepo.record({ actorId: actor.id, action: "content.remove", targetType, targetId, metadata: { reason } });
}

export async function restoreContent(actorId: string, targetType: TargetType, targetId: string): Promise<void> {
  const actor = await permissionsService.requirePermission(actorId, "content.restore");
  if (targetType === "story") {
    await storiesService.moderatorRestoreStory(targetId);
  } else if (targetType === "comment") {
    await engagementService.moderatorRestoreComment(targetId);
  } else {
    throw new HttpError(400, "content.restore only applies to story or comment targets.");
  }
  await moderationActionsRepo.record({ actorId: actor.id, actionType: "restore_content", targetType, targetId });
  await auditRepo.record({ actorId: actor.id, action: "content.restore", targetType, targetId });
}

export async function restrictUserByUsername(actorId: string, username: string, reason: string | null): Promise<void> {
  const actor = await permissionsService.requirePermission(actorId, "users.restrict");
  const target = await usersRepo.findUserByUsername(username);
  if (!target) throw new HttpError(404, "User not found.");
  if (target.isPrimaryAdmin) throw new HttpError(403, "The super admin can't be restricted.");
  await usersRepo.setAccountStatus(target.id, "restricted");
  await moderationActionsRepo.record({ actorId: actor.id, actionType: "restrict_user", targetType: "user", targetId: target.id, reason });
  await auditRepo.record({ actorId: actor.id, action: "user.restrict", targetType: "user", targetId: target.id, metadata: { reason } });
}

export async function unrestrictUserByUsername(actorId: string, username: string): Promise<void> {
  const actor = await permissionsService.requirePermission(actorId, "users.restrict");
  const target = await usersRepo.findUserByUsername(username);
  if (!target) throw new HttpError(404, "User not found.");
  await usersRepo.setAccountStatus(target.id, "active");
  await moderationActionsRepo.record({ actorId: actor.id, actionType: "unrestrict_user", targetType: "user", targetId: target.id });
  await auditRepo.record({ actorId: actor.id, action: "user.unrestrict", targetType: "user", targetId: target.id });
}
...
export async function listModerationHistory(
  ...
  await permissionsService.requirePermission(actorId, "moderation.history.read");
  if (targetType && targetId) return moderationActionsRepo.listForTarget(targetType, targetId, limit, offset);
  return moderationActionsRepo.listAll(limit, offset);
}
...
export async function unsuspendUserByUsername(moderatorId: string, username: string): Promise<void> {
  ...
  const user = await usersRepo.findUserByUsername(username);
  if (!user) throw new HttpError(404, "User not found.");
  await usersRepo.setAccountStatus(user.id, "active");
}
