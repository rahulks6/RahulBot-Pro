...
| FFmpeg / FFprobe                         | **Phase 4:** `EpisodeAssembler` encodes the master; `FfprobeMediaProbe` validates it                                                        | Runs on the app machine's CPU (never a rented GPU). Found via `FFMPEG_PATH` / `FFPROBE_PATH` or the PATH; `ASSEMBLY_MODE` chooses auto / ffmpeg / mock.                                                                                                                                                                                                               |
...
- **Voice Lock**: a locked voice freezes model, identity, reference recording, language, pitch, speed and style. Emotion (happy, sad, whispering…) is a per-line delivery parameter and never changes identity. The narrator is a project-level voice profile.
...
## 18. Episode assembly, AI audio models and voice consent (Phase 4)

- **Assembly** (`services/assembler.ts`, `media/ffmpeg.ts`; all commands are argument arrays, no shell):
  1. Each timeline shot becomes a normalised segment: cover-scale + centre-crop to the exact output size (which also reframes 16:9 footage for 9:16 Shorts), exact fps, trim-in, and the last frame held when speech runs longer than the clip. Mock placeholder stills get a slow push-in.
  2. Segments are joined with cuts, `xfade` crossfades or fade-to-black. A crossfade overlaps into extra frames rendered on the outgoing shot, so the total length (and audio sync) never changes. All inputs are normalised to one timebase first (`settb=AVTB`).
  3. Title-track items are drawn with `drawtext` from text files (`expansion=none`, so no escaping or `%` expansion issues). A TrueType font comes from `TITLE_FONT_FILE` or common system paths; without one, titles are skipped with a warning.
  4. The mix is rendered at the export sample rate (48 kHz default) and normalised with **two-pass EBU R128 loudnorm** (−14 LUFS integrated, −1.5 dBTP by default; linear mode from the first-pass measurement), then AAC stereo.
  5. Mux to MP4 with `+faststart`. The master is copied into storage with a streaming SHA-256 (`assets.createFromFile`), probed with ffprobe (plus a full decode + `volumedetect` pass), and validated by the unchanged spec §52 rules.
  - Findings added: `mock_visuals` (info: a real MP4 whose footage is placeholder stills), `loudness_target` (warn: more than 1 LU off target) and `assembly_warning`.
  - Encoding settings (CRF, x264 preset, AAC bitrate, sample rate, LUFS, true peak, crossfade and fade lengths) live in Settings → Encoding.
- **Mixer**: renders one layer at a time (bounded memory for long episodes) with linear-interpolation resampling to the export rate.
- **Worker adapters** (contract-tested against fakes; the real run is the GPU benchmark):
  - `StableAudioModel` (`adapters/audio_gen.py`): Stable Audio Open via diffusers `StableAudioPipeline` for music beds, SFX and ambience. Up to ~47 s per call; longer music is tiled from sections with new seeds and equal-power crossfades; ambience is made seamlessly loopable; output is mono WAV peak-limited to −1 dBFS. Post-processing is standard-library Python.
  - `SpandrelUpscaler` (`adapters/upscale.py`): Real-ESRGAN (or other super-resolution weights) via `spandrel`, tiled with padding to bound VRAM, then Lanczos to the exact target. Videos are processed frame by frame. Output is always marked _not native resolution_. Weights come from a local file you downloaded (or an explicit Hugging Face filename), never an implicit URL.
  - `CommandLipSync` (`adapters/lipsync.py`): runs a locally installed LatentSync / MuseTalk checkout as a configured argument list (placeholders `{video}`, `{audio}`, `{output}`), no shell, cancellable and time-limited, then normalises the result to H.264 at the source size and validates it with ffprobe. Mock clips are refused.
  - Chatterbox accepts a reference recording (`audio_prompt_path`); Kokoro and the mocks log that they cannot clone.
  - The benchmark suite gained `music` (including a 75 s bed that exercises tiling) and `sfx` (including a loopable ambience) sections.
- **Voice reference with consent** (`services/voice-reference.ts`, migration `0003`):
  - A reference recording (WAV, 3–60 s, ≤ 10 MB) can only be attached with a `voice_consents` record: speaker, relationship (own voice or consenting person), method (self / written / recorded statement / contract), permitted scope and where the evidence is kept. An explicit confirmation is required.
